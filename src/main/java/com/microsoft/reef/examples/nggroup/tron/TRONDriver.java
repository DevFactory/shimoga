...
@DriverSide
@Unit
public class TRONDriver {
  private static final Logger LOG = Logger.getLogger(TRONDriver.class.getName());
    ...
    this.communicationsGroup
        .addBroadcast(ControlMessageBroadcaster.class,
            BroadcastOperatorSpec.newBuilder()
                ...
                .build())
        .addBroadcast(ConjugateDirectionBroadcaster.class,
            BroadcastOperatorSpec.newBuilder()
                .setSenderId(MasterTask.TASK_ID)
                .setDataCodecClass(SerializableCodec.class)
                .build())
        .addReduce(ProjectedDirectionReducer.class,
            ReduceOperatorSpec.newBuilder()
                .setReceiverId(MasterTask.TASK_ID)
                .setDataCodecClass(SerializableCodec.class)
                .setReduceFunctionClass(SumUpIntVecPairReduceFunction.class)
                .build())
        .addReduce(LossSecondDerivativeCompletionReducer.class,
            ReduceOperatorSpec.newBuilder()
                .setReceiverId(MasterTask.TASK_ID)
                .setDataCodecClass(SerializableCodec.class)
                .setReduceFunctionClass(LossSecDerCompletionReduceFunction.class)
                .build())
    ...
    private void submitGroupCommunicationsService(final ActiveContext activeContext) {
      ...
      if (!dataLoadingService.isDataLoadedContext(activeContext)) {
        ...
        serviceConf = Tang.Factory.getTang()
            ...
            .bindImplementation(FeatureParser.class, SVMLightParser.class)
            .build();
      }
  ...
  final class TaskCompletedHandler implements EventHandler<CompletedTask> {
    ...
    public void onNext(final CompletedTask task) {
      ...
      if (retVal != null) {
        final List<Double> losses = TRONDriver.this.lossCodec.decode(retVal);
    ...
    return Tang.Factory.getTang()
        .newConfigurationBuilder(
            TaskConfiguration.CONF
                ...
                .build())
        .bindNamedParameter(ModelDimensions.class, "" + tronControlParameters.getDimensions())
        .bindImplementation(LossFunction.class, tronControlParameters.getLossFunction())
