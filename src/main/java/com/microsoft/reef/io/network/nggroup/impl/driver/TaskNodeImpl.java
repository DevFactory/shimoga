...
public class TaskNodeImpl implements TaskNode {
  ...
  public void onRunningTask() {
    ...
    if (parent != null && parent.isRunning()) {
      final GroupCommunicationMessage gcm = Utils.bldVersionedGCM(groupName, operName, Type.ParentAdd, parent.getTaskId(),
    ...
    for (final TaskNode child : children) {
      if (child.isRunning()) {
        final GroupCommunicationMessage gcm = Utils.bldVersionedGCM(groupName, operName, Type.ChildAdd, child.getTaskId(),
  ...
  public void onParentRunning() {
    ...
    if (parent != null && parent.isRunning()) {
      ...
      final GroupCommunicationMessage gcm = Utils.bldVersionedGCM(groupName, operName, Type.ParentAdd, parentTaskId,
  ...
  public void onParentDead() {
    ...
    if (parent != null) {
      ...
      final GroupCommunicationMessage gcm = Utils.bldVersionedGCM(groupName, operName, Type.ParentDead, parentTaskId,
          parentVersion, taskId, version.get(),
          Utils.EmptyByteArr);
      taskNodeStatus.expectAckFor(gcm.getType(), gcm.getSrcid());
      senderStage.onNext(gcm);
  ...
  public void onChildRunning(final String childId) {
    ...
    if (childTask != null && childTask.isRunning()) {
      ...
      final GroupCommunicationMessage gcm = Utils.bldVersionedGCM(groupName, operName, Type.ChildAdd, childId,
  ...
  public void onChildDead(final String childId) {
    ...
    if (childTask != null) {
      ...
      final GroupCommunicationMessage gcm = Utils.bldVersionedGCM(groupName, operName, Type.ChildDead, childId,
          childVersion, taskId, version.get(),
          Utils.EmptyByteArr);
      taskNodeStatus.expectAckFor(gcm.getType(), gcm.getSrcid());
      senderStage.onNext(gcm);
  ...
  private void sendTopoSetupMsg() {
    ...
    senderStage.onNext(Utils.bldVersionedGCM(groupName, operName, Type.TopologySetup, driverId, 0, taskId,
